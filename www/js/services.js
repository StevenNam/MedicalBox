angular.module('starter.services', [])
  ...
  .factory('OpenFDA', function ($log, Restangular) {
    return {
      searchDrugsByGenericName: function (genericName) {
        return function () {
          genericName = genericName.replace(' ', '+AND+');
          return Restangular.oneUrl('openFDA', 'https://api.fda.gov/drug/event.json?search=(' + genericName.toUpperCase() + ')&count=patient.drug.openfda.generic_name.exact&limit=5').get();
        }
      },
      searchReactionByGenericName: function (genericName) {
        return function () {
          genericName = genericName.replace(' ', '+');
          return Restangular.oneUrl('openFDA', 'https://api.fda.gov/drug/event.json?search=patient.drug.openfda.generic_name.exact:"' + genericName.toUpperCase() + '"&count=patient.reaction.reactionmeddrapt.exact&limit=5').get();
        }
      },
